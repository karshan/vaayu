 ...
 *
 *  Mod by Norbim
 *  V1.0 :smooth sample release, enhanced retrigger, stereo samples
 *  V1.1 :initial wav file support
 *  V1.2 :damper pedal support
 * 
 * ==========================================================================
 */
...
#define CLUSTER_CACHE_SIZE 128*64/NUM_SAMPLES_TO_OPEN // typically for 32 * 64*512 bytes = max sample file length of 1 MB !!!
...
static Audio_FileTypeDef samplefile_type[NUM_SAMPLES_TO_OPEN];	// RAW or WAV file
...
static u8 samplefile_st[NUM_SAMPLES_TO_OPEN];	// Sample file stereo?
...
static u8 samplebyte_buf[POLYPHONY][SAMPLE_BUFFER_SIZE*2];	// Create a buffer for each voice
static u32 sample_cluster_cache[NUM_SAMPLES_TO_OPEN][CLUSTER_CACHE_SIZE];	// Array of sample cluster positions on SD card

static u8 sample_bank_no=0;	// The sample bank number being played
static u8 switch_bank_no=0;	// The sample bank selected via switch for J10
...
void Open_Bank(u8 b_num)	// Open the bank number passed and parse the bank information, load samples, set midi notes, number of samples and cache cluster positions
{
  ...
  WAVE_FormatTypeDef waveformat; //WAV reader template
   ...
   else
  {
		 ...
		 for(samp_no=0;samp_no<no_samples_loaded;samp_no++)	// Open all sample files and mark all samples as off
		 {
		   ...
		   DEBUG_MSG("Open sample file failed.");
			  ...
			  samplefile_type[samp_no]=RAW_FILE;
			  samplefile_st[samp_no]= 0;
			  if (FILE_ReadBuffer((u8 *)&waveformat, sizeof(waveformat))){
			  DEBUG_MSG("Sample file too short.");
			  } else {
				  if ((waveformat.ChunkID==CHUNK_ID) && (waveformat.FileFormat==FILE_FORMAT)) //WAV file
				  {
					  if ((waveformat.AudioFormat==WAVE_FORMAT_PCM) && (waveformat.BitPerSample==BITS_PER_SAMPLE_16)
						  && (waveformat.SampleRate==SAMPLE_RATE_44100)&& (waveformat.NbrChannels<=2)
						  && (waveformat.SubChunk2ID==DATA_ID))
					  {
						  samplefile_type[samp_no]=(Audio_FileTypeDef)waveformat.NbrChannels;
						  DEBUG_MSG("Valid %d channels WAV file.",samplefile_type[samp_no]);
						  samplefile_len[samp_no]=sizeof(waveformat)+ waveformat.SubChunk2Size;
						  samplefile_st[samp_no]=waveformat.NbrChannels-1;
					  } else {
						  DEBUG_MSG("Invalid WAV format.");
						  samplefile_len[samp_no]=0;
					  	  }
				  }
			  }
...
void APP_MIDI_NotifyPackage(mios32_midi_port_t port, mios32_midi_package_t midi_package)
{
  ...
  if( midi_package.chn == midichannel && (midi_package.type == NoteOn || midi_package.type == NoteOff) )	// Only interested in note on/off on chn1
  {
		...
		for(samp_no=0;samp_no<no_samples_loaded;samp_no++)	// go through array looking for mapped notes
		{
			if(midi_package.note==sample_to_midinote[samp_no])		// Midi note on matches a note mapped to this sample samp_no
			{
				...
				if( sample_on[samp_no]<0 )
				{
					...
					{
						sample_on[samp_no]=-3;	//set state to sustain.
					}
  ...
  else if (midi_package.chn==midichannel && midi_package.type==ProgramChange)
    {
		...
		for(samp_no=0;samp_no<voice_no;samp_no++){
			sample_on[voice_samples[samp_no]]=0; // Turn playing samples off
		}
  ...
  else if (midi_package.chn==midichannel && midi_package.type==CC && midi_package.evnt1==64) // Sustain message
  {
   if (midi_package.evnt2>=64)
	   {
	   damper_pedal=1;  // Sustain on
	   DEBUG_MSG("Sustain on");
	   }
   else
   	   {
   	   damper_pedal=0;  // Sustain off
   	   DEBUG_MSG("Sustain off");
   	   for(samp_no=0;samp_no<no_samples_loaded;samp_no++)	// go through array looking for mapped notes
   	   {
			if( sample_on[samp_no]==-3)
			{
				if (!hold_sample[samp_no])	// if not holding sample turn the damping notes off
				{
					sample_on[samp_no]=32767 ; //sample_decval[samp_no];							// Mark it as decaying with the appropriate time for this sample
				}
			}
...
void SYNTH_ReloadSampleBuffer(u32 state)
{
  ...
  MIOS32_BOARD_LED_Set(0x1, 0x1);	// Turn on LED at start of DMA routine
	...
	{
		for(voice=0;voice<voice_no;voice++) 	// read up to SAMPLE_BUFFER_SIZE samples into buffer for each voice
		{
			if(samplefile_pos[voice_samples[voice]] >= samplefile_len[voice_samples[voice]]) // We've reached EOF - don't play this sample next time and also free up the voice
			{
				...
				//DEBUG_MSG("Reached EOF on sample %d at %d",voice_samples[voice],samplefile_pos[voice_samples[voice]]);
			}
			for (j=0;j<=samplefile_st[voice_samples[voice]];j++){
				if(SAMP_FILE_read(&samplebyte_buf[voice][j*SAMPLE_BUFFER_SIZE],SAMPLE_BUFFER_SIZE,voice_samples[voice])<0) // Read in the appropriate number of sectors for each sample thats on
			...
			if(sample_on[voice_samples[voice]]!=0 && samplefile_pos[voice_samples[voice]] > samplefile_len[voice_samples[voice]]) // zeroing the file end
			{
				for(i=(SAMPLE_BUFFER_SIZE*(1+samplefile_st[voice_samples[voice]])-(samplefile_pos[voice_samples[voice]]-samplefile_len[voice_samples[voice]]));i<SAMPLE_BUFFER_SIZE*(1+samplefile_st[voice_samples[voice]]); i++) {	// Fill the sample buffer end with silence
					  samplebyte_buf[voice][i]=0;	// Muted output
					 }
					//DEBUG_MSG("footer mute %d",(samplefile_pos[voice_samples[voice]]-samplefile_len[voice_samples[voice]]));
			}
		...
		for(i=0; i<(SAMPLE_BUFFER_SIZE); i+=2) // Fill half the sample buffer
			{
				...
				{
					j=i*(1+samplefile_st[voice_samples[voice]]); //stereo sample correction
					if (sample_retrigger[voice_samples[voice]]>0){ // if retrigger, short decay
						...
						SampleCalc=((sample_on[voice_samples[voice]])*(s16)((samplebyte_buf[voice][j+1] << 8) + samplebyte_buf[voice][j]))>>15;
						}
						...
						SampleCalc=((samplebyte_buf[voice][j+1] << 8) + samplebyte_buf[voice][j]);
						}
					else SampleCalc=0;
				...
				for(voice=0;voice<voice_no;voice++)
				{
					...
					if (sample_on[voice_samples[voice]]>0){
						SampleCalc=((sample_on[voice_samples[voice]])*(s16)((samplebyte_buf[voice][j+1+(2*samplefile_st[voice_samples[voice]])] << 8) + samplebyte_buf[voice][j+(2*samplefile_st[voice_samples[voice]])]))>>15;
						}
					else if (sample_on[voice_samples[voice]]<0){
						SampleCalc=((samplebyte_buf[voice][j+1+(2*samplefile_st[voice_samples[voice]])] << 8) + samplebyte_buf[voice][j+(2*samplefile_st[voice_samples[voice]])]);
						}
					else SampleCalc=0;
					OutWavs32+=voice_velocity[voice]*(s16)(SampleCalc);		// mix
				}
